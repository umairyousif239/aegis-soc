...
function LoadingScreen({ status }) {
  return (
    <div style={{
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@700&family=DM+Sans:wght@500&family=JetBrains+Mono:wght@400&display=swap');
        @keyframes barPulse {
...
function App() {
  ...
  // WebSocket — only connect once backend is ready
  useEffect(() => {
    if (!ready) return;
    const connect = () => {
